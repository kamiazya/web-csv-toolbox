...
> **Note for Bundler Users**:
> - **Main entry** (`web-csv-toolbox`): WASM auto-initializes from embedded base64. No WASM URL configuration needed.
> - **Slim entry** (`web-csv-toolbox/slim`): Requires explicit `loadWASM(wasmUrl)` with bundler-resolved URL.
> - **Worker URL**: Always required when using Workers with bundlers (both Main and Slim).
>
> See [How to Use with Bundlers](./using-with-bundlers.md) for detailed configuration.
...
```typescript
for await (const record of parse(csv, {
  engine: EnginePresets.recommended() // Worker + WASM (non-blocking)
})) {
  console.log(record);
  // UI stays responsive
}
...
for await (const record of parse(largeCSV, {
  engine: { worker: true, wasm: true } // Single worker, no pool
})) {
  console.log(record);
}
```
...
- [ ] Use `EnginePresets.turbo()` for UTF-8 CSV
...
**Solution:** Use `EnginePresets.recommended()` (Worker + WASM)
...
2. **Use presets** - `EnginePresets.turbo()` for optimal config
