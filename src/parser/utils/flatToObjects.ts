 ...
 *
 * Uses `Object.create(null)` with direct property assignment for optimal
 * performance (4× faster than `Object.fromEntries()`). Creating objects
 * without a prototype chain prevents prototype pollution attacks, even if
 * CSV headers include special keys like `__proto__`, `constructor`, or
 * `prototype`.
 *
...
>(data: FlatDataInput): CSVObjectRecord<Header>[] {
  ...
  for (let r = 0; r < recordCount; r++) {
    ...
    // Use Object.create(null) + direct property assignment for performance
    // 4× faster than Object.fromEntries() while remaining safe from prototype pollution
 ...
 * @example
 * ```typescript
 * const flatResult = wasmParser.parseStringToArraySync(csvData);
