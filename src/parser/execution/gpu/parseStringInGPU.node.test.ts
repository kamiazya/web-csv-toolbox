/**
 * Tests for parseStringInGPU function
 */
...
describe("parseStringInGPU", () => {
  // ============================================================
  // Unit Tests
  // ============================================================

  test("should parse simple CSV string", async ({ gpu, skip }) => {
    ...
    for await (const record of parseStringInGPU(csv)) {
  ...
  test(
    "PBT: should produce identical results to CPU parser for random CSV",
    { timeout: 60000 },
    async ({ gpu, skip }) => {
      skipIfNoWebGPU(gpu, skip);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (headers, rows, eol) => {
            ...
            for await (const record of parseStringInGPU(csvData)) {
