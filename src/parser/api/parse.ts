...
>(
  ...
  if (typeof csv === "string") {
    ...
    yield* parseResponse(
      csv,
      responseOptions as ParseOptions<Header> | undefined,
    ) as AsyncIterableIterator<InferCSVRecord<Header, Options>>;
    ...
    yield* parseRequest(
      csv,
      requestOptions as ParseOptions<Header> | undefined,
    ) as AsyncIterableIterator<InferCSVRecord<Header, Options>>;
  } else if (csv instanceof Blob) {
    yield* parseBlob(csv, options) as AsyncIterableIterator<
      InferCSVRecord<Header, Options>
    >;
  }
}
