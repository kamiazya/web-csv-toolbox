 ...
 */
export async function example3_largeFiles(stream: ReadableStream<Uint8Array>) {
  ...
  console.log(
    `Throughput: ${(records.length / duration).toFixed(0)} records/sec`,
  );
    ...
    onRecord: (record) => {
      try {
        ...
        if (record.fields.length !== 3) {
          throw new Error(`Expected 3 fields, got ${record.fields.length}`);
        }

        // Validate field values
        const [id, name, _value] = record.fields.map((f) => f.value);
