...
describe("GPUBinaryCSVLexer PBT: Token-level GPU vs CPU equivalence", () => {
          ...
          async (numCols, numRows, eol) => {
            ...
            try {
              ...
              for await (const token of gpuLexer.lex(csvBytes)) {
                rawGpuTokens.push(normalizeToken(token as unknown as GPUToken));
              }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (numCols, numRows, eol) => {
            ...
            const rows = fc.sample(csvRowArbitrary(numCols, complexFieldArbitrary), numRows);
            const csvData = rows.map((r) => r.map(escapeField).join(",")).join(eol);
            const csvBytes = new TextEncoder().encode(csvData);
            ...
            try {
              ...
              for await (const token of gpuLexer.lex(csvBytes)) {
                rawGpuTokens2.push(normalizeToken(token as unknown as GPUToken));
              }
              // Convert GPU tokens to Token format (merge Field + Delimiter)
            ...
            try {
              ...
              const wasmLexer = new WasmBinaryCSVLexer({
                ...
                for (const chunk of chunks) {
                  for await (const token of gpuLexer.lex(chunk, { stream: true })) {
                    rawGpuTokens3.push(normalizeToken(token as unknown as GPUToken));
                  }
                }
                // Flush
                ...
                if (gpuTokens.length !== wasmTokens.length) {
                  console.error(`Token count mismatch: GPU=${gpuTokens.length}, WASM=${wasmTokens.length}`);
                ...
                expect(gpuTokens.length).toBe(wasmTokens.length);
                for (let i = 0; i < gpuTokens.length; i++) {
                  if (JSON.stringify(gpuTokens[i]) !== JSON.stringify(wasmTokens[i])) {
                    console.error(`Token mismatch at index ${i}:`);
                    console.error(`  GPU:`, gpuTokens[i]);
                    console.error(`  WASM:`, wasmTokens[i]);
                    console.error(`  Context (GPU ${Math.max(0, i - 2)} to ${Math.min(gpuTokens.length - 1, i + 2)}):`, gpuTokens.slice(Math.max(0, i - 2), Math.min(gpuTokens.length, i + 3)));
                    console.error(`  Context (WASM ${Math.max(0, i - 2)} to ${Math.min(wasmTokens.length - 1, i + 2)}):`, wasmTokens.slice(Math.max(0, i - 2), Math.min(wasmTokens.length, i + 3)));
                  }
                  expect(gpuTokens[i]).toEqual(wasmTokens[i]);
                }
          ...
          async (numCols, numRows, eol) => {
            ...
            try {
              ...
              const wasmLexer = new WasmBinaryCSVLexer({
                ...
                for await (const token of gpuLexer.lex(csvBytes)) {
                  rawGpuTokens4.push(normalizeToken(token as unknown as GPUToken));
                }
                // Convert GPU tokens to Token format (merge Field + Delimiter)
