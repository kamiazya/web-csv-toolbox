...
describe("CSVRecordAssembler", () => {
  describe("constructor validation", () => {
    test("should throw RangeError if maxFieldCount is negative", () => {
      expect(() => new DefaultCSVRecordAssembler({ maxFieldCount: -1 })).toThrow(
        RangeError,
      );
    });
    ...
    test("should throw RangeError if maxFieldCount is not an integer", () => {
      expect(() => new DefaultCSVRecordAssembler({ maxFieldCount: 1.5 })).toThrow(
        RangeError,
      );
    });
    ...
    test("should accept Number.POSITIVE_INFINITY as maxFieldCount", () => {
      expect(
        () =>
          new DefaultCSVRecordAssembler({ maxFieldCount: Number.POSITIVE_INFINITY }),
      ).not.toThrow();
    });
  });
