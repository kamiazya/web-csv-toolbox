import { Field, RecordDelimiter } from "./common/constants";
import { Token } from "./common/types";

export interface HeaderAbsentParserOptions<Header extends ReadonlyArray<string>> {
  header: Header;
}
...
export type ParserOptions<Header extends ReadonlyArray<string>> =
   ...
   * // { name: "Charlie", age: "30" }
   */
export class ParserTransformar<Header extends ReadonlyArray<string>>
  extends TransformStream<Token, Record<Header[number], string>> {
  private row: string[] = [];
  ...
  constructor(options: ParserOptions<Header> = {}) {
    super({
      transform: (
        token: Token,
        controller: TransformStreamDefaultController<Record<(Header)[number], string>>
      ) => {
        switch (token.type) {
          case Field:
            this.row.push(token.value);
            break;
      ...
      flush: (
        ...
        if (this.row.length > 0 && this.header !== undefined) {
          controller.enqueue(
            // @ts-ignore
            Object.fromEntries(
              this.header.map((header, index) => [
