...
describe("OptimizationHint constants", () => {
  ...
  describe("BACKEND_PRIORITY", () => {
    ...
    it("should prioritize WASM first for consistency hint (no GC pauses)", () => {
      expect(BACKEND_PRIORITY.consistency[0]).toBe("wasm");
    });

    it("should prioritize JS first for balanced hint (fastest CPU backend)", () => {
      expect(BACKEND_PRIORITY.balanced[0]).toBe("js");
    });
  ...
  describe("CONTEXT_PRIORITY", () => {
    ...
    it("should prioritize main first for consistency hint (simpler, more predictable)", () => {
      expect(CONTEXT_PRIORITY.consistency[0]).toBe("main");
    });
  ...
  describe("GPU_CONFIG", () => {
    ...
    it("should use balanced workgroup size for consistency hint", () => {
      expect(GPU_CONFIG.consistency.workgroupSize).toBe(64);
      expect(GPU_CONFIG.consistency.devicePreference).toBe("low-power");
    });
