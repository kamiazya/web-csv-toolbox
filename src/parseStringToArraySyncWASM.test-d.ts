...
describe("generics", () => {
  it("should CSV header of the parsed result should be the one specified in generics", () => {
    type Result1 = ReturnType<typeof parseStringToArraySyncWASM<string, ",", "\"", ["name", "age", "city", "zip"]>>;
    expectTypeOf<Result1>().toEqualTypeOf<CSVRecord<["name", "age", "city", "zip"]>[]>();

    type Result2 = ReturnType<typeof parseStringToArraySyncWASM<string, "#", "$", ["name", "age", "city", "zip"]>>;
    expectTypeOf<Result2>().toEqualTypeOf<CSVRecord<["name", "age", "city", "zip"]>[]>();
  });
});
