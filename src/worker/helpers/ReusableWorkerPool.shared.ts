...
/**
 * Type for the createWorker function that creates a Worker instance.
 * @internal
 */
type CreateWorkerFn = (workerURL?: string | URL) => Promise<Worker>;
...
export class ReusableWorkerPool implements WorkerPool, Disposable {
  ...
  protected readonly createWorker: CreateWorkerFn;
  ...
  async getWorker(workerURL?: string | URL): Promise<Worker> {
    ...
    if (totalWorkers < this.maxWorkers) {
      ...
      const workerPromise = this.createWorker(effectiveURL)
        .then((worker) => {
          if (this.disposed) {
