...
// Mock the internal loaders before importing WasmInstance.lite
...
describe("WasmInstance.lite", () => {
  ...
  describe("lite version characteristics", () => {
    it("should verify loadWASM requires parameter", () => {
      // TypeScript enforces that lite version loadWASM requires InitInput
  ...
  describe("comparison with main version", () => {
    it("should have different initialization requirements", () => {
      // Lite version loadWASM signature requires InitInput
      // Main version loadWASM signature has optional InitInput
      // This is enforced by TypeScript types
      const liteLoadWASM: (input: any) => Promise<void> = loadWASM;

      expect(liteLoadWASM).toBeDefined();
      expect(liteLoadWASM.length).toBe(1); // Expects 1 required parameter
    });

    it("should not auto-initialize without explicit call", async () => {
      (loadWASMModule.isInitialized as Mock).mockReturnValue(false);

      // In lite version, WASM is NOT automatically initialized
