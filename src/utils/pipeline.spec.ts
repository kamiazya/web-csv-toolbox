...
const createInputStream = (values: any[]) =>
  new ReadableStream({
    start(controller) {
...
const createToStringTransformer = () =>
  new TransformStream({
    transform(chunk, controller) {
...
const createIdentityTransformer = () =>
  new TransformStream({
    transform(chunk, controller) {
...
describe("pipeline", () => {
  ...
  it("should chain three transformers", async () => {
    ...
    const double = createDoubleTransformer();
    const addOne = createAddOneTransformer();
    const toStringTransformer = createToStringTransformer();

    const result = pipeline(input, double, addOne, toStringTransformer);
